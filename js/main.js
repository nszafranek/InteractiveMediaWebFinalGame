...
function setup() {
  //if mobile
  if (screen.width <= 800) {
    cnv = createCanvas(screen.width - 50, screen.height - 100);
    cnv.id('gameCanvas');
...
function draw() {
  ...
  if ((!gameOver) && (!startGame)) {
    ...
    if (screen.width <= 800) {
      ...
      text("Press Esc to end", (width / 2), (height / 2));
      text("Avoid the Sharks and collect Gems and Coins!", (width / 2), (height / 2) + 20);
      textSize(30);
      strokeWeight(8);
    ...
    if (screen.width > 800) {
      ...
      strokeWeight(5);
      text("Use the Arrow Keys or swipe in one direction to move the hero", (width / 2), (height / 2) - 20);
      text("Press Esc to end", (width / 2), (height / 2) + 40 );
